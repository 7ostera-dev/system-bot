...
var owners = '1094942270995828776'
...
client.login(process.env.token);// التوكن حقك 
// نهايه الاساسيات
//---------------------------------------------------------
// كود الهيلب 
...
**
             ** ✨TICKET COMMAND ✨
             ...
             ${prefix}de  | delete ticket
             ${prefix}cl  | Close chat 
...
client.on('message', niro => {
  if (niro.content.startsWith(prefix + 'add')) {
    ...
    if (the_typed_emoji.id) {
      const link = `https://cdn.discordapp.com/emojis/${the_typed_emoji.id}.${the_typed_emoji.animated ? 'gif' : 'png'
        }`;
...
client.on("message", message => {
  if (message.content.includes("الوان") || message.content.includes("غبي"))
    message.delete();
});
client.on("message", message => {
...
`)
      .setImage('https://geek-post.net/wp-content/uploads/2017/09/21.jpg')
...
`)
      .setImage('https://geek-post.net/wp-content/uploads/2017/09/29.jpg')
    message.channel.send(yassen)
  }
})
...
`)
      .setImage('https://geek-post.net/wp-content/uploads/2017/09/26.jpg')
    message.channel.send(yassen)
  }
})
...
`)
      .setImage('https://geek-post.net/wp-content/uploads/2017/09/11.jpg')
    message.channel.send(yassen)
  }
})
...
`)
      .setImage('https://geek-post.net/wp-content/uploads/2017/09/8-4.jpg')
    message.channel.send(yassen)
  }
})
